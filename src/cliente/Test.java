...
class Test {
	...
	static private boolean prompt(String f) {
		...
		System.out
				.println("\tB indica tamaño para read|write y posicion para seek");
		System.out
				.println("\tNOTA: linea vacia indica pasar al siguiente fichero");
	...
	static public void main(String args[]) {
		...
		try {
			...
			while (true) {
				...
				System.out
						.println("Introduzca nombre de fichero y modo: r (defecto) o rw; (Crtl-D para terminar)");
				if (!ent.hasNextLine())
					return;
				...
				try {
					...
					while (prompt(fich) && ent.hasNextLine()) {
						...
						if (s.hasNextInt()) {
							...
							if (s.hasNextInt()) {
								...
								if (val % tamBloque != 0)
									System.err
											.println("Error: tamaño/posición deben ser múltiplos del tamaño del bloque");
								else
									switch (op) {
									case 0:
										doRead(f, val);
					...
					e.printStackTrace();
					System.out
							.println("Error en operacion E/S sobre el fichero");
				}
			}
		} catch (Exception e) {
